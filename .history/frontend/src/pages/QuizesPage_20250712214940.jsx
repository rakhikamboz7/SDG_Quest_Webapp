...
import TeddyBear from "../components/an"
      ...
      >
        ...
        {!showResult ? (
          <div className="w-full relative">
            ...
            {/* Quiz Card with reduced opacity when teddy is peeking */}
            <motion.div
              className={`w-full bg-white rounded-3xl shadow-xl p-8 relative overflow-visible transition-opacity duration-300 ml-16 ${
                showPeekingOver ? "opacity-80" : "opacity-100"
              }`}
              key={currentQuestion}
              initial={{ x: 100, opacity: 0 }}
              animate={{ x: 0, opacity: showPeekingOver ? 0.8 : 1 }}
              transition={{ duration: 0.5 }}
            >
              {/* Progress Bar */}
              <div className="mb-8">
