...
import AnimatedTeddyBear, { PeekingOverTeddy, ShyPeekingTeddy, StatsTeddyFace } from "../components/AnimatedTeddy"
...
function QuizPage() {
  ...
  // Enhanced teddy bear states
  const [showTeddyBear, setShowTeddyBear] = useState(true)
  const [quizStarted, setQuizStarted] = useState(false)
  const [showPeekingOver, setShowPeekingOver] = useState(false) // Peeking over during quiz
  const [showShyPeeking, setShowShyPeeking] = useState(false) // Shy peeking after completion
  const [showStatsTeddy, setShowStatsTeddy] = useState(false)
  ...
  const handleTeddyAnimationComplete = () => {
    ...
    setQuizStarted(true)
    // Show peeking over teddy when quiz starts
    setTimeout(() => {
  ...
  const handleNext = async () => {
    if (quiz && currentQuestion < quiz.questions.length - 1) {
      ...
      setSelectedOption(null)
      // Peeking over teddy stays visible - no changes needed
    } else {
      // Hide peeking over teddy when quiz completes
      setShowPeekingOver(false)
      setShowResult(true)
  ...
  const saveScore = async (quizScore) => {
    try {
      ...
      // Show celebration or encouraging message based on score
      if (quizScore >= 4) {
        setShowCelebration(true)
        setTimeout(() => setShowCelebration(false), 5000)
        // Show shy peeking teddy for completion
        setTimeout(() => {
        ...
        // Show encouraging shy peeking teddy
        setTimeout(() => {
          setShowShyPeeking(true)
        }, 2500)
      }
  ...
  const handleNextQuiz = () => {
    setShowStatsTeddy(true)
  }

  const handleStatsTeddyComplete = () => {
    setShowStatsTeddy(false)
    const currentQuizIndex = allQuizzes.findIndex((q) => String(q.goalId) === String(goalId))
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-green-50 to-blue-50 relative overflow-hidden">
      {/* Welcome Animated Teddy Bear */}
      {showTeddyBear && (
        <AnimatedTeddyBear goalId={goalId} onAnimationComplete={handleTeddyAnimationComplete} mode="welcome" />
      )}
      ...
      >
        {/* Quiz Header with Stats Teddy overlay */}
        <motion.div
          ...
          transition={{ duration: 0.5 }}
        >
          <StatsTeddyFace show={showStatsTeddy} onComplete={handleStatsTeddyComplete} />
        ...
        {!showResult ? (
          <motion.div
            ...
            transition={{ duration: 0.5 }}
          >
            {/* 🧸 Cute Peeking Over Teddy - stays during entire quiz */}
            <PeekingOverTeddy show={showPeekingOver} />
          ...
          >
            {/* 🎭 Shy Peeking Teddy - appears after completion */}
            <ShyPeekingTeddy goalId={goalId} score={score} show={showShyPeeking} />
