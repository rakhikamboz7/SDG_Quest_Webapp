...
const TeddyBear = ({ show, mode = "peeking", score, onAnimationComplete }) => {
  ...
  // 🎉 BOTTOM RIGHT COMPLETION ANIMATION
  const bottomRightCompletionAnimation = async () => {
    setCurrentMessage(getCompletionMessage())

    // Slide up from bottom with gorgeous effect
    await controls.start({
      y: [100, 0],
      x: [50, 0],
      scale: [0.5, 1],
  ...
  useEffect(() => {
    ...
    if (mode === "welcome") {
      ...
      bottomRightCompletionAnimation()
    }
  }, [show, mode, score])
    ...
    return (
      ...
      >
        ...
        <div className="relative w-48 h-48 drop-shadow-2xl">
          ...
          />
          <div className="absolute right-1/4 top-1/2 w-28 h-12 bg-gradient-to-r from-[#D2691E] to-[#8B4513] border-2 border-[#8B4513] rounded-full transform translate-x-1/2 -translate-y-1/2 rotate-45 origin-bottom shadow-xl" />
  ...
  if (mode === "peeking") {
    return (
      ...
      >
        <div className="relative w-40 h-28 drop-shadow-xl">
          {/* Gorgeous Head */}
          <motion.div
            animate={headControls}
          ...
          <motion.div
            animate={armControls}
            className="absolute left-3 bottom-0 w-12 h-10 bg-gradient-to-br from-[#D2691E] to-[#8B4513] rounded-full border-2 border-[#8B4513] shadow-lg"
          >
  ...
  // 🎉 BOTTOM RIGHT COMPLETION TEDDY (Half visible, peeking from below)
  return (
    <motion.div animate={controls} className="absolute bottom-0 right-4 z-30" style={{ originX: 0.5, originY: 1 }}>
      {/* Magical Glow */}
      <motion.div
        animate={glowControls}
      ...
      {/* Half Teddy - Only upper body visible */}
      <div className="relative w-36 h-24 overflow-hidden">
        {/* Body (partially visible) */}
        <div className="absolute left-1/2 top-8 w-24 h-20 bg-gradient-to-br from-[#D2691E] to-[#8B4513] rounded-full border-2 border-[#8B4513] shadow-xl transform -translate-x-1/2">
        ...
        <motion.div
          animate={armControls}
          className="absolute left-2 top-4 w-20 h-8 bg-gradient-to-r from-[#D2691E] to-[#8B4513] border-2 border-[#8B4513] rounded-full transform -rotate-45 origin-bottom shadow-lg"
        />

        {/* Other Arm */}
        <div className="absolute right-6 top-6 w-16 h-7 bg-gradient-to-r from-[#D2691E] to-[#8B4513] border-2 border-[#8B4513] rounded-full transform rotate-30 shadow-lg" />

        {/* Gorgeous Head */}
        <motion.div
          animate={headControls}
          className="absolute left-1/2 top-0 w-24 h-24 bg-gradient-to-br from-[#D2691E] to-[#8B4513] rounded-full border-3 border-[#8B4513] shadow-2xl transform -translate-x-1/2"
        >
          {/* Cute Ears */}
          <motion.div
            animate={earControls}
            className="absolute left-1 -top-1 w-8 h-8 bg-gradient-to-br from-[#8B4513] to-[#654321] rounded-full shadow-lg"
          >
            <div className="absolute left-1/2 top-1/2 w-5 h-5 bg-gradient-to-br from-[#FFB6C1] to-[#FF69B4] rounded-full transform -translate-x-1/2 -translate-y-1/2"></div>
          </motion.div>
          <motion.div
            animate={earControls}
            className="absolute right-1 -top-1 w-8 h-8 bg-gradient-to-br from-[#8B4513] to-[#654321] rounded-full shadow-lg"
          >
            <div className="absolute left-1/2 top-1/2 w-5 h-5 bg-gradient-to-br from-[#FFB6C1] to-[#FF69B4] rounded-full transform -translate-x-1/2 -translate-y-1/2"></div>
          </motion.div>
          ...
          {/* Extra Rosy Cheeks for happiness */}
          <div className="absolute left-2 top-1/2 w-5 h-5 bg-gradient-to-br from-[#FFB6C1] to-[#FF69B4] rounded-full opacity-80 blur-sm"></div>
          <div className="absolute right-2 top-1/2 w-5 h-5 bg-gradient-to-br from-[#FFB6C1] to-[#FF69B4] rounded-full opacity-80 blur-sm"></div>

          {/* Cute Nose */}
          <div className="absolute left-1/2 top-1/2 w-3 h-2 bg-gradient-to-br from-[#FF69B4] to-[#FF1493] rounded-full transform -translate-x-1/2 shadow-sm"></div>
