...
const TeddyBear = ({ show, mode = "peeking", score, onAnimationComplete }) => {
  ...
  // Peeking animation - diagonal tilt with one paw up
  const peekingAnimation = async () => {
    // Diagonal tilt (hiding behind card)
    await controls.start({
      rotate: -15,
      y: 20,
      x: 20,
  ...
  return (
    <motion.div 
      ...
      className={`fixed z-50 ${
        mode === "peeking" ? "bottom-0 right-0" : 
        mode === "completion" ? "bottom-0 right-0" : 
        "bottom-0 left-0"
      }`}
    ...
    >
      ...
      <div className="relative w-72 h-72">
        ...
        >
          ...
          {/* Nose */}
          <div className="absolute left-1/2 bottom-1/3 w-12 h-10 bg-[#3E2723] clip-path-polygon transform -translate-x-1/2 
                          rounded-[40%]"></div>
          ...
          {/* Mouth */}
          <div className="absolute left-1/2 bottom-1/4 w-6 h-6 border-l-2 border-t-2 border-[#3E2723] 
                          transform -translate-x-1/2 rotate-45"></div>
        </motion.div>
        
        {/* Ears - Positioned higher (top-4) */}
        <motion.div 
          animate={earControls}
